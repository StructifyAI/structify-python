...
class WorkflowResource(SyncAPIResource):
    ...
    def stop(
        ...
        *,
        ...
        return self._post(
            "/workflow/stop",
            body=maybe_transform({"workflow_session_id": workflow_session_id}, workflow_stop_params.WorkflowStopParams),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=NoneType,
    ...
    async def stop(
        ...
        *,
        ...
        return await self._post(
            "/workflow/stop",
            body=await async_maybe_transform(
                {"workflow_session_id": workflow_session_id}, workflow_stop_params.WorkflowStopParams
            ),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
